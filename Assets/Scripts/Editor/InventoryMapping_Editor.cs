...
[CustomEditor(typeof(InventoryMapping))]
public class InventoryMapping_Editor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        for (int i = 0; i < arrayProperty.arraySize; i++)
        {
            ...
            InventoryMappingItem item = ((InventoryMapping)target).mapping[i];
            item.name = EditorGUILayout.TextArea(item.name, GUILayout.Width(200));
            item.sprite = (Sprite)EditorGUILayout.ObjectField(item.sprite, typeof(Sprite));
